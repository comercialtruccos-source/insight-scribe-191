-- =========================================================================
-- CORRECCIÓN Y BACKFILL PARA COLUMNAS "ANO", "ANO_COL" Y FECHAS 2025/2026
-- =========================================================================

-- 1. Actualizar filas donde anio_col o fecha contiene 2025
...
WHERE (anio IS NULL OR anio = 0) AND (
  anio_col LIKE '%2025%'
  OR fecha BETWEEN '2025-01-01' AND '2025-12-31'
  OR fecha_compra BETWEEN '2025-01-01' AND '2025-12-31'
);

-- 2. Actualizar filas donde anio_col o fecha contiene 2026
UPDATE public.fact_ventas
SET 
  anio = 2026,
  mes = COALESCE(mes, EXTRACT(MONTH FROM COALESCE(fecha, fecha_compra))::integer, 1),
...
WHERE (anio IS NULL OR anio = 0) AND (
  anio_col LIKE '%2026%'
  OR fecha BETWEEN '2026-01-01' AND '2026-12-31'
  OR fecha_compra BETWEEN '2026-01-01' AND '2026-12-31'
);

-- 3. Ingest_ventas con soporte explícito para columnas "ano", "ano_col", y extracción de 4 dígitos
