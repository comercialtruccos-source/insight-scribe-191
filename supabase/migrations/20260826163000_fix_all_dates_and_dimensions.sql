-- =========================================================================
-- CORRECCIÓN DEFINITIVA DE FECHAS, AÑOS Y DIMENSIONES EN FACT_VENTAS
-- =========================================================================

-- 1. Asegurar que anio, mes, dia, fecha estén 100% calculados para todos los registros existentes
UPDATE public.fact_ventas
SET 
  fecha = COALESCE(fecha, fecha_compra, (anio::text || '-' || LPAD(COALESCE(mes, 1)::text, 2, '0') || '-' || LPAD(COALESCE(dia, 1)::text, 2, '0'))::date)
WHERE fecha IS NULL AND (fecha_compra IS NOT NULL OR (anio IS NOT NULL AND mes IS NOT NULL));

UPDATE public.fact_ventas
SET 
  anio = COALESCE(anio, EXTRACT(YEAR FROM COALESCE(fecha, fecha_compra))::integer, NULLIF(regexp_replace(COALESCE(anio_col, ''), '\D', '', 'g'), '')::integer),
...
WHERE anio IS NULL OR mes IS NULL OR dia IS NULL;

-- 2. Ingesta con auto-extracción de año, mes, día y fecha
...
BEGIN
  ...
  SELECT
    x.transaccion,
    COALESCE(x.anio, EXTRACT(YEAR FROM COALESCE(x.fecha, x.fecha_compra))::integer, NULLIF(regexp_replace(COALESCE(x.anio_col, ''), '\D', '', 'g'), '')::integer) AS anio,
    COALESCE(x.mes, EXTRACT(MONTH FROM COALESCE(x.fecha, x.fecha_compra))::integer, 1) AS mes,
    COALESCE(x.dia, EXTRACT(DAY FROM COALESCE(x.fecha, x.fecha_compra))::integer, 1) AS dia,
    COALESCE(x.fecha, x.fecha_compra, 
      CASE WHEN x.anio IS NOT NULL AND x.mes IS NOT NULL 
        THEN (x.anio::text || '-' || LPAD(x.mes::text, 2, '0') || '-' || LPAD(COALESCE(x.dia, 1)::text, 2, '0'))::date 
        ELSE NULL END
    ) AS fecha,
...
-- 3. Función RPC para años disponibles
